<?php
namespace LancerHe\Library\Convert;

/**
 * Class Spell
 *
 * @package LancerHe\Library\Convert
...
class Spell {
    ...
    public function getInitials($str, $big2Gb = false) {
        ...
        if ( $this->_isAscii($str[0]) && $this->_isAsciis($str) )
            return $str;
        if ( $big2Gb )
            $str = Big2gb::big2gb($str);
        $result = [];
        if ( $this->__charset == 'utf-8' ) {
    ...
    private function _search($code) {
        $data  = array_keys($this->__pinYinList);
        $lower = 0;
        $upper = sizeof($data) - 1;
        // 排除非一级汉字
