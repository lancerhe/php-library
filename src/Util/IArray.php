...
class IArray {
     ...
     * @param string $sortField  排序键名
     * @param int    $sortType   升序降序
     * @return array
     * @example $array = array(
     ...
     */
    public static function multiSort($multiArray, $sortField, $sortType = SORT_ASC) {
        if ( ! is_array($multiArray) )
            trigger_error('mutisort(): The first parameter should be array', E_USER_WARNING);
        foreach ( $multiArray as $row ) {
            if ( ! is_array($row) ) return $multiArray;
            $arr_field[] = $row[$sortField];
        }
        array_multisort($arr_field, $sortType, $multiArray);
        return $multiArray;
    }
}